...
public class EventManager {
    ...
    public boolean addEventProvider(EventProvider provider) {
        // Checks if the provider is already in the list
        for (EventProvider existing : this.eventProviders) {
            if (existing.getIdentifier().equals(provider.getIdentifier())) {
                // If it is, don't add it again --> return false
                return false;
            }
        }
        // If it is not, add it to the list --> return true
    ...
    public boolean removeEventProvider(String providerId) {
        // Does essentially the same as the addEventProvider method
        // but in reverse
